...
public class UserServiceImpl implements UserServices {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if ("PATIENT".equals(u.getRole())) {
            Patients patient = this.patientRepo.getPatientByUserId(u.getId());
            if (patient != null) {
                u.setPatients(patient);
            }
        }
    ...
    public Users updateUser(int userId, Map<String, String> params, MultipartFile avatar) {
        ...
        if (params.containsKey("fullName")) {
            existingUser.setFullName(params.get("fullName"));
        }
        if (params.containsKey("email")) {
            existingUser.setEmail(params.get("email"));
        }
        if (params.containsKey("phoneNumber")) {
            existingUser.setPhoneNumber(params.get("phoneNumber"));
        }
        if (params.containsKey("gender")) {
            existingUser.setGender(params.get("gender"));
        }
